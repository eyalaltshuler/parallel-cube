...
public class DistGenerator extends Configured implements Tool {
	...
	*/
	...
	public static class SortGenMapper1 extends Mapper<LongWritable, NullWritable, Text, Text> {
	    ...
	    private ArrayList<Element> generateRow() {
	    	...
	    	for (int i=0; i<colsNum; i++) {
	    		result.add(new Element(random.nextInt(dist)));
	    	}
	    ...
	    public void setup(Context context) {
	    	...
	    	String distAsStr = context.getConfiguration().get("dist");
	    	this.dist = Integer.parseInt(distAsStr);
	    }
	...
	public int run(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
	    ...
	    job.getConfiguration().set("dist", args[4]);
	    return job.waitForCompletion(true) ? 0 : 1;
	}

	  public static void main(String[] args) throws Exception {
	    int res = ToolRunner.run(new Configuration(), new DistGenerator(), args);
	    System.exit(res);
	  }
